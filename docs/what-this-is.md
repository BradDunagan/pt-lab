...
If AI denoise is on, the export is denoised too: it snapshots the color `ImageData` (plus albedo/normal aux buffers at the export resolution when aux is enabled), `await`s the UNet to load, runs it, and writes the denoised result. Convergence maps to 0–85% of the progress bar and denoising to 85–100%. It falls back to a raw export (with a console warning) if WebGPU is missing or aux capture fails.

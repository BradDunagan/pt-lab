...
Scenes are named and saved to localStorage (**New** / **Save…** / **Export** / **Delete**). **Export** downloads the current scene as `<name>.pt-scene.json` — the same `SceneData` that Save stores, pretty-printed, so a scene can be kept in a repo or moved between browsers (localStorage is per origin, and the port counts: `localhost:5173` and `localhost:5174` are different stores). Objects are referenced by library key, and an import's key only resolves in the browser that imported it — so Export also downloads the `.glb` of every **included** imported model, and each imported object in the JSON carries `glb` (that file's name, from `modelFileName`: the library name made file-system-safe, plus `-` and the first 12 hex digits of the bytes' SHA-256) and `sha256`. The hash in the name means two different models that share a name can't overwrite each other in one directory, and a model used by several scenes exports to one file. Export refuses, before downloading anything, if an included import has no hash (only where Web Crypto is unavailable — a plain-http page) or no bytes in this browser; the browser may ask once to allow several downloads. A host outside this browser gets the models back with `registerImport(key, name, bytes)`, under the scene's own keys, before `applyScene` — see [Where the editor's data lives](#where-the-editors-data-lives). There is no import button yet — paste an exported scene back with `localStorage.setItem('pt-lab.scenes', …)` or `lab.applyScene(data)` in the console. A new scene is a room with its light and no objects. The render-mode Scene dropdown lists the built-in demos (which reload) plus saved scenes (which apply live, rebuilt from scratch by `applyScene` so loading works identically from any starting point). Two storage tiers: the **object library** (Table/Cube/Ball + imports) persists globally; each **scene** stores only per-object inclusion/material/transform keyed by library key, plus its lights (scenes saved before lights existed load with none).
...
Each import gets a generated key, `import-<base36 timestamp>-<counter>`, and scenes reference objects by that key. The key is minted per import, so the *same* `.glb` imported in another browser — or re-imported after clearing site data — gets a *different* key, and a scene exported from elsewhere won't resolve it. That is what an exported scene's `glb`/`sha256` fields and `registerImport(key, name, bytes)` are for: a host with the exported files registers each model **in memory only** under the key the scene uses, then calls `applyScene`. It never writes to IndexedDB — a host that persisted every model it rendered would reload them all on its next `init()`, and what it renders would start to depend on what it rendered before — and it refuses a key already in the library rather than replace it. It resolves with the SHA-256 of what it registered, so the host can confirm it matches the scene. A saved key with no library entry is otherwise skipped silently by `applyScene`, which walks the library, not the scene. Hashes are computed when an import is added or loaded, so they cost nothing at save time. To inspect the library:
